...
public class MissingPosterManagerScript : MonoBehaviour
{
	...
	private void Start()
	{
		while (this.ID < 101)
		{
			...
			{
				GameObject gameObject = Object.Instantiate<GameObject>(this.MissingPoster, base.transform.position, Quaternion.identity);
				...
				gameObject.transform.localPosition = new Vector3(-16300f + (float)(this.ID * 500), Random.Range(1300f, 2000f), 0f);
