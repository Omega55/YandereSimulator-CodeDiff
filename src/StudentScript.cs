...
public class StudentScript : MonoBehaviour
{
					...
					if (this.DistanceToDestination < 5f)
					{
						...
						else if (this.DistanceToDestination < this.TargetDistance)
						{
							...
							if (this.DistractTimer <= 0f)
							{
								...
								this.Private = false;
								this.DistractionTarget.CurrentDestination = this.Destinations[this.Phase];
								this.DistractionTarget.Pathfinding.target = this.Destinations[this.Phase];
			...
			if (this.SeekingMedicine)
			{
				...
				else if (this.SeekMedicinePhase == 1)
				{
					...
					if (this.DistanceToDestination < 1f)
					{
						...
						studentScript.RetreivingMedicine = true;
						studentScript.Pathfinding.speed = 0f;
						studentScript.CameraReacting = false;
						studentScript.TargetStudent = this;
				...
				else if (this.SeekMedicinePhase == 2)
				{
					...
					this.targetRotation = Quaternion.LookRotation(new Vector3(studentScript2.transform.position.x, base.transform.position.y, studentScript2.transform.position.z) - base.transform.position);
					base.transform.rotation = Quaternion.Slerp(base.transform.rotation, this.targetRotation, 10f * Time.deltaTime);
					...
					if (this.MedicineTimer > 5f)
					{
						...
						studentScript2.Pathfinding.canSearch = true;
						studentScript2.Pathfinding.canMove = true;
						studentScript2.RetrieveMedicinePhase++;
					}
				}
	...
	private void UpdateTalkInput()
	{
		if (this.Prompt.Circle[0].fillAmount == 0f)
		{
			...
			{
				bool flag = false;
				if (this.Armband.activeInHierarchy && (this.Actions[this.Phase] == StudentActionType.ClubAction || this.Actions[this.Phase] == StudentActionType.SitAndSocialize || this.Actions[this.Phase] == StudentActionType.Socializing || this.Actions[this.Phase] == StudentActionType.Sleuth || this.Actions[this.Phase] == StudentActionType.Lyrics || this.Actions[this.Phase] == StudentActionType.SitAndEatBento) && ((base.transform.position.y > this.StudentManager.ClubZones[(int)this.Club].position.y - 1f && base.transform.position.y < this.StudentManager.ClubZones[(int)this.Club].position.y + 1f && Vector3.Distance(base.transform.position, this.StudentManager.ClubZones[(int)this.Club].position) < this.ClubThreshold) || Vector3.Distance(base.transform.position, this.StudentManager.DramaSpots[1].position) < 12f))
				{
					flag = true;
					this.Warned = false;
				}
				...
				else
				{
					...
					else
					{
						...
						else if (!this.Scrubber.activeInHierarchy && !flag2)
						{
							this.SmartPhone.SetActive(true);
						}
						this.ChalkDust.Stop();
		...
		if (this.Prompt.Circle[2].fillAmount == 0f && this.ClubActivityPhase < 16)
		{
			...
			if (!flag3 && !this.Yandere.NearSenpai && !this.Yandere.Attacking && this.Yandere.Stance.Current != StanceType.Crouching)
			{
