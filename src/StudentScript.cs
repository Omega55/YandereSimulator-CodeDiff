...
public class StudentScript : MonoBehaviour
{
	...
	public void Start()
	{
		if (!this.Started)
		{
			...
			else if (this.Persona == PersonaType.SocialButterfly || this.Persona == PersonaType.Lovestruck || this.Persona == PersonaType.PhoneAddict)
			{
			...
			if (this.StudentID != 1)
			{
				...
				{
					this.FollowTarget = this.StudentManager.Students[33];
					this.PhotoPatience = 0f;
				}
			...
			else if (this.Club == ClubType.Occult)
			{
				...
				{
					if (this.StudentManager.Students[17] == null && this.StudentManager.Students[18] == null)
					{
			...
			if (this.StudentID == 1 || this.StudentID == 19 || this.StudentID == 32 || this.StudentID == 33)
			{
		...
		if (this.Routine)
		{
			...
			else if (this.Phase < this.ScheduleBlocks.Length - 1)
			{
				if (this.Clock.HourTime >= this.ScheduleBlocks[this.Phase].time && !this.InEvent && !this.Meeting)
				{
					...
					if (this.Actions[this.Phase] == StudentActionType.Bully && !this.StudentManager.Bully)
					{
						if (this.BullyID == 1)
						{
					...
					else if (this.Persona == PersonaType.PhoneAddict && !this.Slave)
					{
				...
				if (this.CurrentDestination != null)
				{
					...
					else
					{
						...
						if (this.FollowTargetDistance < 1f && !this.FollowTarget.Alone)
						{
							...
							float num2 = Quaternion.Angle(base.transform.rotation, this.StudentManager.Hangouts.List[this.StudentID].rotation);
						...
						{
							this.targetRotation = Quaternion.LookRotation(this.StudentManager.Students[33].transform.position - base.transform.position);
							base.transform.rotation = Quaternion.Slerp(base.transform.rotation, this.targetRotation, 10f * Time.deltaTime);
						}
					}
					if (!this.Hurry)
	...
	private void UpdateTalkInput()
	{
		if (this.StudentID > 1)
		{
			...
			if ((this.Alarm > 0f || this.AlarmTimer > 0f || this.Yandere.Armed || this.Waiting || this.InEvent) && !this.Slave && !this.BadTime && !this.Yandere.Gazing)
			{
				this.Prompt.Circle[0].fillAmount = 1f;
			}
			if (this.Prompt.Circle[0].fillAmount == 0f)
	...
	private void UpdateWitnessedMurder()
	{
		if (!this.Fleeing)
		{
			...
			else
			{
				...
				if ((double)this.CharacterAnimation[this.PhoneAnims[4]].time > 3.66666)
				{
	...
	private void UpdateAlarmed()
	{
		...
		if (this.WitnessedCorpse)
		{
			...
			{
				Debug.Log(base.name + " is walking backwards");
	...
	private void UpdateVomiting()
	{
		if (this.Vomiting)
		{
			...
			if (this.VomitPhase == 0)
			{
				if (this.DistanceToDestination < 0.5f)
				{
					if (this.StudentID == 33 || this.StudentID == 34)
					{
						this.Prompt.Label[0].text = "     Drown";
	...
	private void WitnessMurder()
	{
		...
		if (this.SmartPhone.activeInHierarchy && this.Persona != PersonaType.Heroic && this.Persona != PersonaType.Dangerous && this.Persona != PersonaType.Evil && !this.Teacher)
		{
			this.Persona = PersonaType.PhoneAddict;
			this.SprintAnim = this.PhoneAnims[2];
		}
	...
	{
		if (!this.Indoors && this.WitnessedMurder && this.StudentID != this.StudentManager.RivalID && this.Persona != PersonaType.Evil && this.Persona != PersonaType.Heroic && this.Persona != PersonaType.Coward && this.Persona != PersonaType.PhoneAddict && this.Persona != PersonaType.Protective)
		{
			this.Persona = PersonaType.Loner;
		}
		if (!this.WitnessedMurder)
	...
	public void GetDestinations()
	{
		...
		while (this.ID < this.JSON.Students[this.StudentID].ScheduleBlocks.Length)
		{
			...
			{
				this.Destinations[this.ID] = this.StudentManager.Students[33].transform;
			}
