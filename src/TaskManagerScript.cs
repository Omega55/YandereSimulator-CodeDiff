...
public class TaskManagerScript : MonoBehaviour
{
		...
		if (!this.Yandere.Talking)
		{
			if (Globals.GetTaskStatus(32) == 1)
			{
				if (this.Yandere.Inventory.Cigs)
				{
					...
					Globals.SetTaskStatus(32, 2);
				}
			}
			else if (Globals.GetTaskStatus(32) == 2 && !this.Yandere.Inventory.Cigs)
			{
				if (this.StudentManager.Students[32] != null)
				{
					this.StudentManager.Students[32].TaskPhase = 4;
				}
				Globals.SetTaskStatus(32, 1);
			}
		}
	}
	...
	public void UpdateTaskStatus()
	{
		...
		if (Globals.GetTaskStatus(7) == 1 && this.StudentManager.Students[7] != null && this.StudentManager.Students[7].TaskPhase == 0)
		{
			this.StudentManager.Students[7].TaskPhase = 4;
		}
		if (Globals.GetTaskStatus(13) == 1 && this.StudentManager.Students[13] != null)
		{
			this.StudentManager.Students[13].TaskPhase = 4;
			for (int i = 1; i < 26; i++)
			{
				if (Globals.GetKittenPhoto(i))
				{
					this.StudentManager.Students[13].TaskPhase = 5;
				}
			}
		}
		if (Globals.GetTaskStatus(14) == 1)
		{
		...
		else if (Globals.GetTaskStatus(14) == 2 && this.StudentManager.Students[14] != null)
		{
			this.StudentManager.Students[14].TaskPhase = 5;
		}
		if (Globals.GetTaskStatus(15) == 1)
		{
			if (this.StudentManager.Students[15] != null)
			{
